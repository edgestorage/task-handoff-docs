...
const socialImages = {
  en: `${siteOrigin}/images/workbench-overview-en.png`,
  zh: `${siteOrigin}/images/workbench-light-mode.png`
}
...
function localizedRoutes(route: string) {
  ...
  const contentRoute = chinese || english ? route.slice(3) || '/' : route
  const englishRoute = contentRoute === '/' ? '/' : `/en${contentRoute}`
  const chineseRoute = contentRoute === '/' ? '/zh/' : `/zh${contentRoute}`

  return { chinese, chineseRoute, englishRoute }
}
...
function descriptionFromSource(filePath: string, title: string) {
  ...
  const english = !filePath.startsWith('zh/')
...
type SidebarGroup = { text: string; items: Array<{ text: string; link: string }> }

function sidebarAtPrefix(sidebar: SidebarGroup[], prefix: string, removePrefix = '') {
  return sidebar.map((group) => ({
    ...group,
    items: group.items.map((item) => {
      const route = removePrefix && item.link.startsWith(removePrefix)
        ? item.link.slice(removePrefix.length) || '/'
        : item.link

      return { ...item, link: `${prefix}${route}` }
    })
  }))
}

const zhSidebar = sidebarAtPrefix(zhSidebarRoutes, '/zh')
const enSidebar = sidebarAtPrefix(enSidebarRoutes, '')
...
export default defineConfig({
    ...
    transformItems: (items) => items.filter((item) => {
      ...
      return {
        ...
        links: [
          ...
          { lang: 'x-default', url: absoluteUrl(englishRoute) }
        ]
      }
    })
  },
  ...
  transformHead({ pageData, title, description }) {
    ...
    const { chinese, chineseRoute, englishRoute } = localizedRoutes(route)
    const englishHomeAlias = route === '/en/'
    const canonical = absoluteUrl(englishHomeAlias ? '/' : route)
    const locale = chinese ? 'zh_CN' : 'en_US'
    const alternateLocale = chinese ? 'en_US' : 'zh_CN'
    const inLanguage = chinese ? 'zh-CN' : 'en-US'
    const socialImage = chinese ? socialImages.zh : socialImages.en
    const socialImageSize = chinese ? { width: '1960', height: '1423' } : { width: '2178', height: '1664' }
    const isHome = englishRoute === '/'
    ...
    return [
      ...
      ['meta', { name: 'robots', content: englishHomeAlias ? 'noindex, follow' : 'index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1' }],
      ...
      ['meta', { property: 'og:image:alt', content: chinese ? 'TaskHandoff 工作台' : 'TaskHandoff workbench' }],
      ['meta', { name: 'twitter:card', content: 'summary_large_image' }],
      ['meta', { name: 'twitter:title', content: title }],
      ['meta', { name: 'twitter:description', content: description }],
      ['meta', { name: 'twitter:image', content: socialImage }],
      ['meta', { name: 'twitter:image:alt', content: chinese ? 'TaskHandoff 工作台' : 'TaskHandoff workbench' }],
  ...
  vite: {
    plugins: [
      {
        name: 'task-handoff-english-aliases',
        configureServer(server) {
          server.middlewares.use((request, response, next) => {
            if (!request.url) return next()

            const url = new URL(request.url, 'http://localhost')
            const legacyChinesePrefixes = [
              '/admin/', '/agents/', '/automation/', '/clients/', '/guide/', '/instances/',
              '/integrations/', '/reference/', '/security/', '/sessions/', '/settings/',
              '/tutorials/', '/workspace/'
            ]
            const legacyChinesePages = ['/faq', '/manual', '/troubleshooting']
            const legacyChinese = legacyChinesePages.includes(url.pathname)
              || legacyChinesePrefixes.some((prefix) => url.pathname.startsWith(prefix))
            if (!legacyChinese) return next()

            const targetPath = `/zh${url.pathname}`
            response.statusCode = 302
            response.setHeader('Location', `${base.replace(/\/$/, '')}${targetPath}${url.search}`)
            response.end()
          })
        }
      }
