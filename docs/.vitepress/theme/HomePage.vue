...
const props = defineProps<{ locale?: 'zh' | 'en'; pathPrefix?: string }>()
...
const copy = {
  zh: {
    ...
    previews: [
      { label: '工作台', image: '/images/workbench-light-mode.png', alt: 'TaskHandoff Story 工作台：组织文档、会话与自动化', detail: '把文档、会话和自动化，放回同一个任务里', width: 1960, height: 1423 },
      { label: 'AI 会话', image: '/images/ai-session-view-modes.png', alt: 'TaskHandoff AI 会话：查看当前任务和工具活动', detail: '聚焦当前回合，执行过程与完整历史随时可查', width: 1960, height: 1424 },
      { label: '远程节点', image: '/images/node-management-overview.png', alt: 'TaskHandoff 节点管理：统一查看本机与远程运行环境', detail: '环境可以分布各处，管理入口始终只有一个', width: 1960, height: 1424 }
    ],
  ...
  en: {
    ...
    previews: [
      { label: 'Workbench', image: '/images/workbench-overview-en.png', alt: 'TaskHandoff workbench with the instance sidebar and AI workspace', detail: 'Instances, sessions, and controls. Together in one workbench', width: 2178, height: 1664 },
      { label: 'AI sessions', image: '/images/instance-detail-en.png', alt: 'TaskHandoff instance with an English AI session composer', detail: 'Choose a workspace and agent, then start the next task', width: 2350, height: 1664 },
      { label: 'Stories', image: '/images/story-overview-en.png', alt: 'TaskHandoff Story with actions, documents, sessions, and automation', detail: 'Keep long-running work and its context in one place', width: 2350, height: 1664 }
    ],
    ...
    docs: [
      ['/guide/', 'Get oriented', 'Understand nodes, instances, sessions, and the workbench'],
      ['/guide/install', 'Install TaskHandoff', 'Choose a Desktop or server deployment'],
      ['/sessions/', 'Run AI sessions', 'Create, supervise, and recover an AI task'],
      ['/automation/', 'Build automation', 'Use Stories to orchestrate repeatable workflows']
    ],
...
const prefix = computed(() => props.pathPrefix ?? (props.locale === 'zh' ? '/zh' : ''))
  ...
  <main ref="home" class="th-home">
        ...
        <div class="th-product__layout" data-reveal>
          ...
          <div class="th-product__stage">
            <figure id="preview-image" class="th-media">
              <img :key="preview.image" :src="withBase(preview.image)" :alt="preview.alt" :width="preview.width" :height="preview.height" loading="lazy">
            </figure>
    ...
    <section class="th-section th-docs">
      <div class="th-shell th-docs__layout">
        ...
        <div class="th-doc-grid"><a v-for="item in content.docs" :key="item[0]" :href="withBase(`${prefix}${item[0]}`)" data-reveal><div><strong>{{ item[1] }}</strong><p>{{ item[2] }}</p></div><span aria-hidden="true">↗</span></a></div>
      </div>
    </section>
    ...
    <footer class="th-footer">
      <div class="th-shell th-footer__grid">
        <div class="th-footer__brand"><a :href="withBase(locale === 'en' ? '/' : `${prefix}/`)" class="th-footer__logo"><img :src="withBase('/brand/task-handoff-light.svg')" alt="" width="30" height="30" loading="lazy">TaskHandoff</a><p>{{ content.footerDescription }}</p></div>
