...
1. 确认 [Control Plane 公网地址](/zh/integrations/public-control-plane) 可由目标机器访问。
...
::: info 账号型节点中继
添加节点向导会显示 Thandoff 账号选项的能力说明，但当前协议尚未支持账号型 Node Agent 中继。这里的替代路径是可信控制面代理，不是 [Thandoff 账户远程访问](/zh/integrations/thandoff-account)。
:::
